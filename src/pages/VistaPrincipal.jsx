...
const VistaPrincipal = () => {
  ...
  const previousVideoRef = useRef(videoSrc);
  const { subtitulo, titleColor, contenedoricons, icons } = ColorTitulo(
    data.temperatura
  );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const newData = generateData();
      const newVideoSrc = VideoTemperatura(newData.temperatura);

      // Solo realizar la transición si el video realmente cambia
      if (newVideoSrc !== previousVideoRef.current) {
        setCambioVideo(true);
        previousVideoRef.current = newVideoSrc;
        ...
        // Cambiar video y datos después del fade out
        setTimeout(() => {
        ...
        // Si el video no cambia, solo actualizar los datos sin transición
        setData(newData);
      }
    }, 5000);
    ...
    <div className="flex flex-wrap gap-4 p-2 bg-[#FFFFF2] text-white min-h-screen">
      ...
      >
        ...
        <CardContent className="relative z-10 p-6 h-full flex flex-col justify-between">
          ...
          <div
            style={{ backgroundColor: contenedoricons }}
            className="flex justify-around rounded-lg w-[22.313rem] h-[6rem] items-center transition-all duration-700 ease-in-out"
          >
            <MetricaSimple
              icon={<Droplets className="w-[1.25rem] h-[1.25rem]" />}
              value={`${data.humedad}%`}
...
const MetricaSimple = ({ icon, value, label, color }) => {
  return (
    <div
      style={{ color }}
      className="flex flex-col items-center transition-colors duration-700 ease-in-out"
    >
      {icon}
      <div className="text-2xl font-bold mt-2 text-[1.563rem]">{value}</div>
      <div className="text-[0.563rem]">{label}</div>
